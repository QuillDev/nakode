...
use crate::{
    backend::{
        ...
        BackendIdentity, BackendOperation, CapabilitySupport, KIMI_PROVIDER, ModelInfo,
...
const DEFAULT_CONTEXT_WINDOW: usize = 262_144;
...
impl BackendConfig {
    ...
    pub fn native(workspace: PathBuf) -> Self {
        Self {
            workspace,
            credential: None,
            base_url: KIMI_BASE_URL.to_owned(),
            client: Client::new(),
...
impl InferenceProvider for KimiProvider {
    fn infer(
        &self,
...
impl KimiProvider {
    async fn infer_response(
        ...
        let mut last_error = InferenceAttemptError::terminal("Kimi inference failed");
    ...
    async fn infer_attempt(
        ...
        let response = self
            ...
            .json(&kimi_request_body(request))
            .send()
            .await
            .map_err(|error| {
                InferenceAttemptError::transient(format!("Kimi request failed: {error}"))
            })?;
        if !response.status().is_success() {
            ...
            return Err(InferenceAttemptError {
                message: format!("Kimi returned {status}: {detail}"),
                retryable: retryable_status(status),
                retry_after,
            });
        }
        parse_kimi_sse(response, events, cancellation).await
    }
}

/// Starts the in-process Kimi Coding Plan adapter.
///
/// # Errors
///
...
async fn run_supervisor(
    ...
    let session_store = config
        .session_database
        .clone()
        .map(|database| RuntimeSessionStore::new(database, KIMI_PROVIDER));
...
async fn api_key_auth_required(events: &mpsc::Sender<BackendEvent>) {
    request_failed(
        ...
        "Kimi uses an API key configured in provider settings",
    )
    .await;
}
...
async fn start_session(
    ...
    let Some(selected) = selected else {
        request_failed(
            context.events,
            BackendOperation::StartSession,
            "Kimi returned no usable models",
        )
        .await;
        return;
    };
...
async fn discover_models(
    ...
    if !response.status().is_success() {
        let status = response.status();
        let detail = response.text().await.unwrap_or_default();
        return Err(format!("Kimi model discovery returned {status}: {detail}"));
    }
    let payload: Value = response
        .json()
        .await
        .map_err(|error| format!("invalid Kimi model catalog: {error}"))?;
    let entries = payload
        .get("data")
        .or_else(|| payload.get("models"))
        .and_then(Value::as_array)
        .ok_or_else(|| "Kimi returned an invalid model catalog".to_owned())?;
    let mut models = entries
        ...
        .collect::<Vec<_>>();
    if models.is_empty() {
        return Err("Kimi returned no usable Coding Plan models".to_owned());
    }
...
fn kimi_request_body(request: &InferenceRequest) -> Value {
    ...
    if request.model == "k3" || request.model == "k3-256k" || request.model.starts_with("kimi-k3") {
        body["reasoning_effort"] =
            Value::String(kimi_reasoning_effort(request.reasoning_effort.as_deref()));
    }
    body
}

fn kimi_reasoning_effort(effort: Option<&str>) -> String {
...
async fn parse_kimi_sse(
    ...
    loop {
        ...
        let chunk = chunk.map_err(|error| {
            let message = format!("Kimi stream failed: {error}");
        ...
        while let Some(boundary) = pending.find('\n') {
            ...
            let event: Value = serde_json::from_str(data).map_err(|error| {
                InferenceAttemptError::terminal(format!("invalid Kimi event: {error}"))
            })?;
            apply_kimi_event(&event, &events, &mut output, &mut tool_calls)
                .await
    ...
    if !pending.trim().is_empty() {
        return Err(InferenceAttemptError::terminal(
            "Kimi stream ended in the middle of an event",
        ));
    }
    if !completed {
        return Err(InferenceAttemptError::terminal(
            "Kimi stream ended before a completion marker",
        ));
    }
...
async fn apply_kimi_event(
    ...
    if let Some(message) = event.pointer("/error/message").and_then(Value::as_str) {
        return Err(format!("Kimi stream error: {message}"));
    }
...
mod tests {
    ...
    use super::{
        KimiProvider, ToolCallAccumulator, apply_kimi_event, context_window_for_model,
        kimi_reasoning_effort, kimi_request_body,
    };
    use crate::runtime::{
    ...
    fn request_uses_chat_messages_and_fixed_sampling_parameters() {
        let body = kimi_request_body(&request());
        assert_eq!(body["model"], "k3-256k");
        ...
        assert_eq!(kimi_reasoning_effort(Some("xhigh")), "max");
    }
    ...
    async fn stream_events_normalize_text_reasoning_tools_and_usage() {
        ...
        apply_kimi_event(&json!({"id":"chat-1","choices":[{"delta":{"reasoning_content":"think","content":"done",
            "tool_calls":[{"index":0,"id":"call-1","function":{"name":"read","arguments":"{\"path\":"}}]}}]}),
            &events, &mut output, &mut calls).await.expect("first event");
        apply_kimi_event(&json!({"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\"README.md\"}"}}]}}],
    ...
    async fn provider_sends_bearer_authenticated_chat_request_and_parses_sse() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert!(request.contains("\"model\":\"k3-256k\""));
            let payload = concat!(
