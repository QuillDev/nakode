...
Use `transport discord status` and Nakode logs for redacted configuration/runtime state. Status exposes only whether a valid bounded token is configured, never its value.

## Current readiness

**NO-GO for credentialed live testing as of this audit.** The exact final source passes the deterministic repository gates: Nakode format/check, warning-denied Clippy, all-target/all-feature build, the 671-test locked all-target/all-feature suite, and the headless Agent TUI scenario; FStack dashboard format/typecheck, all desktop/web/shot/main builds, Discord tests (8/8), Chat lifecycle (10/10), Agent pane (42/42), and Nakode Agent integration tests (33/33) also pass. The Excalidraw JSON sources and SVG XML validate, and both PNG previews were visually inspected. This is substantial offline evidence, but it is not evidence of Discord's credentialed HTTP/gateway behavior.

The blocking architecture gap is installation-wide ownership. Every enabled workspace service starts a Serenity gateway for the shared token. Events fan out and are filtered against workspace-local mappings, while thread uniqueness and event admission are enforced only inside each workspace database. If two workspace authorities ever overlap on one thread, both can durably admit the same event. The Identify lease limits session starts but does not elect one gateway owner or prevent route overlap.
...
Credentialed testing should wait for an installation-wide route/event ownership invariant (or a single gateway owner plus shared dispatcher), then close the named transport fault gaps in deterministic adapters first. No bot credential is available in the development environment, so no live Discord API/gateway behavior has been exercised. Do not interpret deterministic coverage as live validation.
