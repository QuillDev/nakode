...
The turn catalogue is copied into native provider runtimes before prompt construction and tool execution. `read_skill` reads only that in-process catalogue: it does not rescan the filesystem and a literal `/skill:name` cannot override disabled or unavailable state. A changed catalogue is therefore visible on the next turn; a turn already executing retains the catalogue with which it started.
