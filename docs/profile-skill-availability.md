...
Clients may call the typed `PruneSkill` service/SDK mutation after checking the separate `SkillPruning` capability. The request identifies exactly one stable skill ID and carries a profile ID as authenticated/profile-scoped catalogue context, but successful cleanup is machine-local and global for that stable identity.

Nakode fails closed unless the stable identity is a retained, unavailable, removable record. Unknown identities and every currently discovered/installed definition are rejected. Consequently, pruning cannot silently delete an available package, immutable or built-in definition, or installed filesystem content. For an installed row, the authoritative restriction tells the owner to remove the package first, complete a successful discovery refresh, and only then prune its retained unavailable record.

The repository performs one transaction that deletes only:

- every `skill_preferences` row with the exact stable skill ID across all profiles;
- `invocation_events` where `kind = 'skill' AND identity = <stable skill ID>`; and
- `invocation_aggregates` with that same exact predicate.

Telemetry for another skill and telemetry of another kind that happens to use the same identity are preserved. Telemetry remains opt-in, local-only, privacy-minimal, and owned by Nakode; pruning does not enable collection or broaden consent. After commit, Nakode removes the identity from all in-memory profile preference caches and reinstalls skill authority for loaded sessions.
...
Older clients ignore the new protobuf fields and continue to list and enable installed skills. Updated clients connected to older servers decode conservative defaults (`prunable = false`) and must hide/disable prune controls because `SkillPruning` is absent. No database migration is required: reconciliation and cleanup use the existing `skill_preferences`, `invocation_events`, and `invocation_aggregates` tables.
