...
- Durable linked-child completion/blocker/question evidence enters the persisted inbox with runtime-owned origin and deduplicated receipts; native runs never cause extra parent turns. Stop/close and uncertain dispatch remain explicit recovery boundaries. `docs/child-followup-delivery.md`.
- Authenticated source-call relays derive downward delegated instruction from current canonical same-owner parent links; peer/upward content stays inert. Structured batch display and exact-turn bounded final reports persist independently of model text. `docs/coordination-messages.md`.
- Durable claim/transfer uses exact source-call authentication, same-profile parent/revision fences and immutable transition receipts; unsettled child evidence or delegated instructions prevent reassignment. `docs/session-relationships.md`.
...
Session process environment is write-only, memory-only server state keyed by exact live or retained logical session identity. Injection never activates providers and precedes explicit restoration. Native Bash, PTY tools, delegated tools and owner shell commands inherit it without provider-context or snapshot serialization; successful durable deletion clears it, while failed deletion retains it.
