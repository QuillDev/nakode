...
async fn command_sent_before_initialize_is_deferred_not_dropped() -> TestResult {
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let fixture = manifest.join("tests/fixtures/fake_codex.py");
    // The fixture inherits the parent's temporary-directory environment. Keep the FIFO in that
    // same root so its confinement check remains deterministic in isolated test harnesses.
